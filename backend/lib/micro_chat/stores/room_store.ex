defmodule MicroChat.RoomStore do
  ...
  def handle_call({:add_user, room_id, user_id}, _from, state) do
    ...
    updated_room = %{room | users: Map.put(room.users, user_id, %{ice_candidates: []})}
  ...
  def handle_call({:set_user_ice_candidates, room_id, user_id, ice_candidates}, _from, state) do
    ...
    updated_room = %{room | user_id => %{user_state | ice_candidates: ice_candidates}}
    true = :ets.update_element(@table, room_id, updated_room)
    {:reply, :ok, state}
  end
