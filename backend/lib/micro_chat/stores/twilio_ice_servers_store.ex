defmodule MicroChat.Store.TwilioIceServersStore do
  ...
  @impl true
  def init(_state) do
    {:ok, state} = refresh_ice_servers()
    {:ok, state}
  end

  @impl true
  def handle_info(:refresh_ice_servers, current_state) do
    case refresh_ice_servers() do
      {:ok, new_state} ->
        {:noreply, new_state}

      {:error, error} ->
        Logger.error(
          "Unable to refresh twilio ice servers with reason: #{inspect(error, pretty: true)}"
        )

        # Try again
        Process.send_after(self(), :refresh_ice_servers, 30 * 1000)

        {:noreply, current_state}
    end
  end

  defp refresh_ice_servers do
    case @twilio_api_client.get_ice_servers_and_credentials() do
      {:ok, %{"ttl" => ttl, "ice_servers" => ice_servers}} ->
        ...
        Process.send_after(self(), :refresh_ice_servers, refresh_time)
